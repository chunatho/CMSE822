...
void decrypt32( unsigned char *inp, uint32_t key, unsigned char *decoded, unsigned int *seed) {
...
//        polarity = rand_r(&seed[thread_num]) % 2;
//        magnitude = rand() % 32;
...
#pragma omp for
   for (i=Rprocess; i<Nmax; i+=Nprocess) {
        ...
        decrypt32( encrypted, i, decrypted, &seed[thread_num] );
