...
//    srand(key);
    ...
    while (     (a = inp[iend++]) != 0
        ...
        polarity = rand_r(&seed) % 2;
        magnitude = rand_r(&seed) % 32;
        ...
        xor =   (rand_r(&seed) % 256 << 24) | (rand_r(&seed) % 256 << 16)
              | (rand_r(&seed) % 256 << 8)  | (rand_r(&seed) % 256);
    ...
    if ( a != 0 && b != 0 && c != 0 && d == 0 ) {
        ...
        xor = (rand_r(&seed) % 256 << 16) | (rand_r(&seed) % 256 << 8) | (rand_r(&seed) % 256);
        ...
        xor = (rand_r(&seed) % 256 << 8) | (rand_r(&seed) % 256);
...
//{
//int seed = Rprocess*thread_count+omp_get_thread_num();
//int *p = &seed;
//printf("process %d thread %d has seed: %d \n", Rprocess, omp_get_thread_num(), (*p));

//printf("process %d thread %d checking in \n",Rprocess);
#pragma parallel omp for num_threads(thread_count)
   for (i=Rprocess; i<Nmax; i+=Nprocess) {
        if(loopflag) continue;
        decrypt32( encrypted, i, decrypted );
        strcpy(dcopy, decrypted);
        if (isValid(dcopy, len)) {
              printf("process %d thread %d has valid key: %d \n", Rprocess, omp_get_thread_num(), i);
              success = 1;
              valid = 0;
	...
	if(((i-Rprocess)/Nprocess)%CHECK_INT==0){
           printf("process checking %d", Rprocess);
   ...
   if(0==success){
      printf("process %d has failed to find the key, for its honor it is commiting sepuku \n",Rprocess);
      MPI_Finalize();
      return 0;
   }
