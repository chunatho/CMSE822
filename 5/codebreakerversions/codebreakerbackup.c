...
//printf("Launching pigeon post for %d \n", Rprocess);
...
//      if(i==299436)printf("process %d has key: %d \n",Rprocess,i);
    int64_t start = Nmax/Nprocess*Rprocess; int64_t stop = Nmax/Nprocess*(Rprocess+1);
    if(Rprocess+1==Nprocess)stop=Nmax;
    for (i=Nmax/Nprocess*Rprocess; i<Nmax/Nprocess*(Rprocess+1); i++) {
//        if(i==Rprocess)printf("starting %d \n",i);  

        decrypt32( encrypted, i, decrypted );
//        if(i==299436)printf("process %d just checked key: %d \n",Rprocess,i);

        strcpy(dcopy, decrypted);
//        if(i==299436)printf("process %d has len= %d \n",Rprocess,len);
